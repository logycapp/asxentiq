    ...
    <div class="container py-4">
      ...
      <div *ngIf="result" class="card">
        <div class="card-body text-center py-5">
          <div *ngIf="result.passed" class="display-1 text-success mb-3">&#10004;</div>
          <div *ngIf="!result.passed" class="display-1 text-danger mb-3">&#10008;</div>

          <h3 class="mb-2">{{ result.training.title }}</h3>

          <div class="my-4">
            <span class="display-4 fw-bold" [class.text-success]="result.passed" [class.text-danger]="!result.passed">
              {{ result.score ?? 'N/A' }}%
            </span>
          </div>

          <div class="mb-3">
            <span [class]="'badge fs-5 py-2 px-4 ' + (result.passed ? 'bg-success' : 'bg-danger')">
          ...
          <div class="mt-4">
            <button class="btn btn-primary me-2" (click)="downloadCertificate()">
  ...
  downloadCertificate(): void {
    ...
    this.loadingService.track(this.trainingService.downloadCertificate(this.result.training.id))
      .subscribe({
        ...
        error: () => {
          this.error = 'Error al descargar el certificado.';
        }
      });
  }
}